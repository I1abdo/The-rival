...
<?php
if(login == 1){
    if(action == add_page){require_once"Include-Files/new_page.php";exit;}
    if(action == Delete_page){require_once'Include-Files/Delete/delete_page.php';exit;}    /*********************************************************************************************************************************************/


// include your code to connect to DB.

    $tbl_name="ri_pages";        //my  table name
...
<form action="pages.php" method="post">
    <div class="page-header">
        <div class="row">
            <h3 class="col-lg-1 col-md-1 col-sm-1">الصفحات </h3>
            ';
        if(user_ulv == 6){echo'<a href="pages.php?action=add_page" style="margin-top: 15px;margin-right: 20px" class="btn btn-sm btn-success">أضف صفحة </a>';}
            echo'            
...
<div class="table-responsive table-p">
    ...
    if($num > 0)
    {
        ...
        {
            /****[ إستحضار الكاتب ]****/
            $Select_author_p = mysqli_query($connect,"SELECT * FROM ri_users WHERE u_id = '".$row2['page_author']."'");
            $Fetch_author_p  = mysqli_fetch_object($Select_author_p);
            echo'
            dddddddd
              <tr>
                <td style="width:4%"><input class="chck_box" name="check_post[]" value="'.$row2['ID'].'" type="checkbox"/></td>
                <td style="width:45%">'.$row2['page_title'].'</td>
                <td style="width:15%">'.$Fetch_author_p->user_login.'</td>
                <td style="width:10%">'.$row2['page_date'].'</td>
                ...
                <td><a href="pages.php?action=Delete_page&id='.$row2['ID'].'"><span class="glyphicon glyphicon-trash"></span></a></td>
              </tr>
            ';
        };
    }else
    {
            echo'
                <tr>
                    <td style="width:9%"><p style="font-family: tahoma">لا توجد صفحات.</p></td>
                    <td></td>
                    <td></td>
                    <td></td>
                    <td></td>
    ...
    if(isset($_POST['delete_group'])){
        ...
        for($i=0;$i<$num;$i++){
            $del_id= $check[$i];
            $sql5 = "DELETE FROM ri_pages WHERE ID = '".$del_id."'";
